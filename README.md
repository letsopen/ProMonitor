...
- 存储用 **SQLite 单文件**（纯 Go 驱动，无 cgo），进程内内存做 10 分钟均值聚合，**高频原始数据不落库**。
...
```
被控 Agent (Go, 每 30s)
   │  HTTP POST /api/ingest  + HMAC-SHA256 验签
   ▼
主控 Server (Go :9000)
   ├─ 内存聚合窗口（每被控 10 分钟均值，不落原始数据）
   ├─ 每 10 分钟结算 1 行 → SQLite metrics_agg（保留 30 天）
   ├─ 实时刷新 latest_snapshot（列表/详情读它，极轻）
   └─ 托管前端 dist/  +  REST API
            │
            ▼
...
**上报可靠性**：Agent 内置内存积压队列，上传失败按指数退避自动重试（最多 5 次）；
网络故障期间数据暂存队列（上限 500 条，超限丢最旧），恢复后自动续传。
...
- 原始高频上报**只进内存聚合窗口，绝不逐条落库**，对服务端资源极友好。

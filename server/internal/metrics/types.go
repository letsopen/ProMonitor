...
// 合法延迟上限为 1000ms（agent 与服务端聚合均以 v <= 1000 计入均值，>1000 视为无效），
// 因此 3000ms 足够区分超时且不会误伤任何合法样本。
// 选 3000 而非 9999：y 轴上限随之降到 ~3150ms，正常低延迟曲线（几十 ms 级）不再被
// 高位哨兵压到横轴上无法分辨。
const PingTimeoutMs = 3000.0
